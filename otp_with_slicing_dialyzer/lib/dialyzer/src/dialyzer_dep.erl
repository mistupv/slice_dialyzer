...
set__is_element(El, #set{set = Set}) ->
  ...
  sets:is_element(El1, Set).

strip_ann(El) ->
  %io:format("Petamiento: ~p\n",[El]),
  cerl:set_ann(El, [{label, cerl_trees:get_label(El)}]).
...
state__new(Tree) ->
  Exports = set__from_list(lists:map(fun strip_ann/1, [X || X <- cerl:module_exports(Tree)])),
  %io:format("\nExports: ~p\n~P\n",[Exports,cerl:module_defs(Tree),12]),
