...
-export([depth/1, fold/3, free_variables/1, get_label/1, label/1, label/2, 
	 map/2, mapfold/3, size/1, variables/1,get_lbls_from_tree/1,
	 get_ln_from_labels/2]).%,get_tree_from_lbl/2]).
...
label(T, N, Env) ->
    case type(T) of
 	literal ->
	    ...
	    {set_ann(T, As), N1};
	var ->
		%It should be like literals in order to have different labels for each ocurrence
%	    case dict:find(var_name(T), Env) of
%		{ok, L} ->
%		    {As, _} = label_ann_(T, L),
%		    N1 = N;
%		error ->
%		    {As, N1} = label_ann_(T, N)
%	    end,
%	    {set_ann(T, As), N1};
...
label_ann(T, N) ->
    {[{label_slicing, N} | filter_labels(get_ann(T))], N + 1}.
   

filter_labels([{label_slicing, _} | As]) ->
    filter_labels(As);
filter_labels([A | As]) ->
    [A | filter_labels(As)];
filter_labels([]) ->
...
postorder(F,Tree) ->
     case cerl:subtrees(Tree) of
          [] -> [F(Tree)];
          List -> [F(Tree)] ++ [[postorder(F, Subtree) || Subtree <- Group] || Group <- List]  
     end.
...
get_lbls_from_tree(Tree) -> lists:sort(lists:flatten(postorder(fun get_label_node/1, Tree))).

%%%%%%%%%%%%%%%%%%

postorder(_,_,[]) -> [];
postorder(F,Tree,Lbls) ->
        case cerl:subtrees(Tree) of
            [] -> [F(Tree,Lbls)];
            List -> [F(Tree,Lbls) | begin
%                                       case cerl:is_c_call(Tree) of
%                                            true -> io:format("subtrees call: ~p\n",[List]),
%                                                    io:format("postorder: ~p\n",[[[postorder(F, Subtree,Lbls) || Subtree <- Group] || Group <- List]]);
%                                            _ -> ok
%                                       end,
                                       Result = lists:flatten([[postorder(F, Subtree,Lbls) || Subtree <- Group] || Group <- List]),
				       case cerl:is_c_call(Tree) of
				            true -> case lists:flatten(List) of
				                         [T1,T2|_] -> 
				                            case cerl:is_literal(T1) and cerl:is_literal(T1) of
				                                 true -> join_call_name(Result,T1,T2);
				                                 _ -> Result
				                             end;
				                         _ -> Result
				                    end ;
...
join_call_name([{File,{Pos,Node}}|Tail],T1,T2)->
   case Node of 
        T1 -> {c_literal,Ann,MName} = T1,
              {c_literal,_,FName} = T2,
              [{File,{Pos,{c_literal,Ann,atom_to_list(MName)++":"++atom_to_list(FName)}}}
               |join_call_name(Tail,T1,T2)];
        T2 ->  Tail;
        _ -> join_call_name(Tail,T1,T2)
   end;
join_call_name([_|Tail],T1,T2)-> join_call_name(Tail,T1,T2);
join_call_name([],_,_) -> [].

get_label2(_,[]) -> false;		
get_label2(Node,[Lbl|Lbls]) ->
    case get_ann(Node) of
		[_,{label_slicing, Lbl},Pos,{file,File} | _] when is_integer(Pos) -> {File,{Pos,Node}};
		[{label_slicing, Lbl},Pos,{file,File} | _] when is_integer(Pos) ->  {File,{Pos,Node}};
		[_,{label_slicing, _} | _] -> get_label2(Node,Lbls);
		[{label_slicing, _}| _] -> get_label2(Node,Lbls);		
		_ -> false
    end.   
    
get_ln_from_labels(Tree,Lbls) -> to_dict(lists:filter(fun(X) -> X /= false end,postorder(fun get_label2/2, Tree,Lbls)),dict:new()).

to_dict([],Dict)-> Dict;
to_dict([X|Xs],Dict)-> 
     %io:format("\n\n[X|Xs]:~p\nDict:~p\n\n",[[X|Xs],dict:to_list(Dict)]),
     case X of
	  {Key,{Pos,Lbl}} ->
	      NValue =
	       case dict:find(Key,Dict) of
	            {ok,ValueD} ->
	               case [PosD || {PosD,_} <- ValueD, PosD == Pos] of
	                    [] -> [{Pos,[Lbl]}]++[Entry || Entry = {PosD,_} <- ValueD, PosD /= Pos];
	                    _ ->  [{PosD,LblD++[Lbl] } || {PosD,LblD} <- ValueD, PosD == Pos] ++ [Entry || Entry = {PosD,_} <- ValueD, PosD /= Pos] 
	                          
	               end;
	             _ -> [{Pos,[Lbl]}]
	       end,
	       to_dict(Xs,dict:store(Key,NValue,Dict));
            _ -> to_dict(Xs,Dict)
     end.
...
%postorder_search(F,Tree,Lbl) ->
%     case cerl:subtrees(Tree) of
%          [] -> [];
%          List -> [F(Tree,Lbl)] ++ [[postorder_search(F, Subtree,Lbl) || Subtree <- Group] || Group <- List]  
%     end.
%      
%
%		
%get_label_node(Node,Lbl) ->
%    %io:format("Node:~p\n",[Node]),
%    case get_label(Node) of
%         Lbl -> [Node];		
%	 _ -> []
%    end.   
%
%get_tree_from_lbl(Tree,Lbl) -> lists:sort(lists:flatten(postorder_search(fun get_label_node/2, Tree,Lbl))).
